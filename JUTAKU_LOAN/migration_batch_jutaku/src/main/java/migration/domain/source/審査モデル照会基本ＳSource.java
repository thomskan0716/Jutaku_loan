...
/**
 * Source: 審査モデル照会基本Ｓ (Scoring model inquiry - basic S) - SZB_SMS.
 * 1:N per (申込番号, 申込目的), keyed by イベント/イベント日時.
 */
public class 審査モデル照会基本ＳSource {

    private String 申込番号;          // VARCHAR2(12)
    private String 申込目的;          // VARCHAR2(2)
    private String イベント;          // VARCHAR2(50)
    private Date イベント日時;        // DATE
    private BigDecimal 申込時年齢;    // NUMBER(3,0)
    private BigDecimal 実行時年齢;    // NUMBER(3,0)
    private BigDecimal 本人年収;      // NUMBER(13,0)
    private BigDecimal 合算者年収;    // NUMBER(13,0)
    private String 合算方法;          // VARCHAR2(2)
    private BigDecimal 申込金額;      // NUMBER(13,0)
    private BigDecimal 借入期間;      // NUMBER(3,0)
    private String 資金使途;          // VARCHAR2(2)
    private BigDecimal 勤続年数;      // NUMBER(3,0)
    private String 資本金区分;        // VARCHAR2(2)
    private BigDecimal 年間返済額;    // NUMBER(13,0)
    private BigDecimal 総借入額;      // NUMBER(13,0)
    private BigDecimal 自己資金;      // NUMBER(13,0)
    private BigDecimal 所要資金;      // NUMBER(13,0)
    private String 規定外項目;        // VARCHAR2(1)
    private BigDecimal 担保評価額;    // NUMBER(13,0)
    private String 保証料区分;        // VARCHAR2(1)
    private BigDecimal 先順位控除額;  // NUMBER(13,0)
    private BigDecimal 控除前担保評価額; // NUMBER(13,0)
    private BigDecimal 同居家族数;    // NUMBER(4,0)
