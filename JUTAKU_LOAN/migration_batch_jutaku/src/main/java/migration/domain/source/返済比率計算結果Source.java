...
/**
 * Source: 返済比率計算結果 (Repayment-ratio calculation result) - SZB_SMS.
 * 1:N per (申込番号, 申込目的). A base group plus seven credit-source groups
 * (申告 / 今回 / 自行 / ＫＳＣ / ＣＩＣ / ＣＣＢ / ＪＩＣＣ); amounts are NUMBER(13,0),
 * ratios are VARCHAR2.
 */
public class 返済比率計算結果Source {
    ...
    // --- base group ---
    private BigDecimal 計算年収;         // NUMBER(13,0)
    private BigDecimal 借入総額;         // NUMBER(13,0)
    private BigDecimal 無担保借入額;     // NUMBER(13,0)
    private BigDecimal 年間返済額;       // NUMBER(13,0)
    private String 総額借入比率;         // VARCHAR2
    private String 無担保借入比率;       // VARCHAR2
    private String 総額返済比率;         // VARCHAR2
    private BigDecimal 無担保年間返済額; // NUMBER(13,0)
