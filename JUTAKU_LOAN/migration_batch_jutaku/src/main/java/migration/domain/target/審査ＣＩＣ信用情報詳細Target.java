...
/**
 * Target: 審査ＣＩＣ信用情報詳細 (Review CIC credit info detail) - ITF_SMS.
 * 申込番号 first digit 2→3; 申込目的 converted by service. Other columns pass through.
 * DB judgement columns use full-width digits (判断項目名１ ...); Java properties keep half-width.
 */
public class 審査ＣＩＣ信用情報詳細Target {
    ...
    private Integer 詳細連番;    // NUMBER(3,0)
    private Date 受付日時;       // DATE
    private String 受付番号;     // CHAR(12)
    private String 会員名;       // VARCHAR2(4)
    private Integer 回答通番;    // NUMBER(3,0)
    private String 電文種別;     // VARCHAR2(2)
